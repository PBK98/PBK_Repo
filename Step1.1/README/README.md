...
5. 트러블슈팅 2건 이상(문제 -> 원인 가설 -> 확인 -> 해결/대안)
    ...
    3) docker-compose.yml에 볼륨 명시 했으나 인식하지 못하는 문제 발생
        - docker-compose.yml에 바운드 마운팅을 의도하여 volume을 상대 경로로 작성
        - docker-compose up -d 를 수행한 터미널의 현재 경로의 문제로 예상
        - 현재 경로는 맞았으나 AI에게 질문하니 반드시 절대경로를 이용해야 한다고 답변함
        - 절대경로로 수정 후 문제 해결
        
6. attatch 와 exec의 차이
    1) attatch
        - docker에서 실행 중인 컨테이너에 접속하는 명령어, 컨테이너의 터미널에서 빠져나오면 해당 컨테이너는 종료됨 (Ctrl +P, Q 로 나오면 유지)
    
    2) exec
        - 터미널에 진입하지 않고 명령을 실행 ex) docker exec ubuntu-server ls /Users/bumkyu84259392/PBK_Repo/Step1.1/Volumes/UbuntuVolume -> data
        - 터미널에 진입하고 싶으면 -it 와 /bin/bash 사용 ex) docker exec -it ubuntu-server /bin/bash
